...
<div className="space-y-3">
  {filteredWaitlist.map((item) => {
    const clientName = item.client || "לקוח";
    const date = item.requestedDate || "";
    const time =
      item.requestedTime && item.requestedTime !== "any"
        ? item.requestedTime
        : "כל שעה";
    const service = item.note || "שירות";

    return (
      <div
        key={item.id}
        className="w-full p-4 rounded-xl border border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#151515] flex items-center justify-between"
      >
        {/* צד שמאל – טקסט */}
        ...
        {/* צד ימין – כפתור פעולות */}
        <div className="relative">
          <button
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-gray-200 dark:border-[#2b2b2b] bg-white dark:bg-[#181818] text-[12px] font-medium text-gray-800 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-[#222222]"
            onClick={(e) => {
              e.stopPropagation();
              ...
              <button
                ...
                onClick={() => {
                  setWaitlistItems((prev) =>
                    prev.map((w) =>
                      w.id === item.id ? { ...w, status: "booked" } : w
                    )
                  );
