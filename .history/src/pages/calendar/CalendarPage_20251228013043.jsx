          ...
          <div className="relative z-20 flex-1 overflow-y-auto px-6 pt-2 pb-5 text-sm text-gray-800 dark:text-gray-100" dir="rtl">
            ...
            <div className="space-y-3">
              {filteredWaitlist.map((item) => (
                <div
  key={item.id}
  className="w-full p-4 rounded-xl border border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#151515] mb-3 flex flex-col gap-1"
>
  {/* שם הלקוח */}
  ...
  {/* תאריך + שעה + שירות */}
  <div className="text-[13px] text-gray-600 dark:text-gray-300 flex gap-2 flex-wrap">
    <span>{item.date}</span>
    <span>•</span>
    <span>{item.time ?? "ללא שעה"}</span>
    <span>•</span>
    <span>{item.service ?? "ללא שירות"}</span>
  </div>
  ...
  <div className="mt-2">
    <button
      onClick={(e) => {
        e.stopPropagation();
        setOpenWaitlistActionId(prev => prev === item.id ? null : item.id);
      }}
      className="px-3 py-1.5 rounded-full border text-[13px] border-gray-300 dark:border-[#3a3a3a] hover:bg-gray-100 dark:hover:bg-[#242424] transition flex items-center justify-center gap-1"
    >
      פעולות
    ...
    {openWaitlistActionId === item.id && (
      <div
        className="mt-2 bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-200 dark:border-[#333] p-2 text-sm"
        onClick={(e) => e.stopPropagation()}
      >
        <button className="w-full text-right py-1 hover:text-black dark:hover:text-white transition"
          onClick={() => {/* קבע תור */}}>
          קבע תור
        </button>
        <button className="w-full text-right py-1 hover:text-red-500 transition"
          onClick={() => {/* הסרה */}}>
          הסר מרשימה
        </button>
      </div>
