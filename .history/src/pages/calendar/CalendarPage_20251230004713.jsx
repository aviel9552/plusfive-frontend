...
const CalendarPage = () => {
  ...
  // clickedSlot: { staffId, staffName, date, time }

  // 🔥 מפעיל את פופאפ קביעת התור כשלוחצים על סלוט ביומן
const handleCalendarSlotClick = (isoDate, time, staffId, staffName) => {
  setClickedSlot({
    dateIso: isoDate,  // תואם למה שה-AppointmentDrawer מצפה
    time,
    staffId,
    staffName,
  });

  setNewAppointmentService(null);
  setNewAppointmentClient(null);
...
const AppointmentDrawer = ({
  ...
  const handleSave = () => {
    ...
    // פה אתה מגדיר את מבנה האירוע החדש שלך ביומן
    const newEvent = {
      id: Date.now(),
      date: slot.dateIso,       // "YYYY-MM-DD"
      startTime: slot.time,     // "HH:MM"
      // אפשר לחשב שעה סיום לפי משך שירות, כאן רק דוגמה:
      endTime: slot.time,
      staff: slot.staffId,
      ...
      // תוסיף כאן שדות לפי מה שכבר יש לך ב-customEvents
    };
  ...
  return (
    <>
      {/* רקע שקוף לסגירה בלחיצה על המסך */}
      <div
        className="fixed inset-0 bg-black/30 z-40"
        onClick={onClose}
      />

      {/* Drawer מצד ימין ב-RTL */}
      <div
        className="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-[#111111] shadow-2xl z-50 flex flex-col"
        dir="rtl"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200/70 dark:border-gray-800">
          ...
          <button
            onClick={onClose}
            className="ml-2 -mr-2 p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#181818] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#FF257C] focus:border-transparent"
            >
          ...
          <div className="space-y-2">
            ...
            {/* כפתור "הוסף לקוח חדש" – כרגע רק placeholder */}
            <button
              type="button"
              className="mt-2 text-xs font-medium text-[#FF257C] hover:underline"
              onClick={() => {
                // כאן בעתיד תפתח מודאל Add client
                alert("פה נחבר את מודאל ה-Add Client הקיים");
              }}
            >
              + הוסף לקוח חדש
            </button>
        ...
        {/* כפתורי פעולה */}
        <div className="px-6 py-4 border-t border-gray-200/70 dark:border-gray-800 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 h-10 rounded-xl border border-gray-300 dark:border-gray-700 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
          >
            ביטול
          </button>
          <button
            type="button"
            onClick={handleSave}
            className="flex-1 h-10 rounded-xl bg-[#FF257C] text-sm font-semibold text-white hover:opacity-90 transition disabled:opacity-50"
