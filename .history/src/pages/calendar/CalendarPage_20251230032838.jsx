...
// מפעיל את Drawer קביעת התור כשלוחצים על סלוט ביומן
...
const handleDayMouseLeave = () => {
  ...
  const handleDayColumnClick = (e, iso, staff) => {
    ...
    // כמה דקות עברו מתחילת היום
    const minutesFromStart = (y / slotHeight) * 60;
    const rawTotalMinutes = START_HOUR * 60 + minutesFromStart;

    // עיגול ל־15 דקות
    const STEP = 15;
    const minMinutes = START_HOUR * 60;
    const maxMinutes = END_HOUR * 60 - STEP;

    let rounded = Math.round(rawTotalMinutes / STEP) * STEP;
    if (rounded < minMinutes) rounded = minMinutes;
    if (rounded > maxMinutes) rounded = maxMinutes;

    const timeLabel = minutesToLabel(rounded); // "HH:MM"

    handleCalendarSlotClick(iso, timeLabel, staff.id, staff.name);
  };
  ...
  onMouseMove={() => {}}     // ביטול זמני של hover
  onMouseLeave={() => {}}    // ביטול זמני של hover
...
<div className="grid grid-cols-7 border-b border-gray-200 dark:border-commonBorder bg-gray-50/80 dark:bg-[#080808]">
  ...
  const renderWaitlistPanel = () => {
    ...
    setWaitlistAddStep("date");   // ← במקום "client"
    setBookingSelectedTime("any");
    setBookingSelectedService(null);
    setServiceSearch("");
    setIsTimeDropdownOpen(false);
    setSelectedWaitlistClient(null);
    setAddFlowMode("calendar");
      ...
      <div className="fixed inset-0 z-50 flex justify-end">
        ...
        >
          ...
          <div className="px-8 pt-7 pb-3">
            {/* 🔥 שלבי הפלואו – בסדר חדש: תאריך → שעה → שירות → לקוח */}
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-1">
          ...
          <div className="text-[14px] text-gray-500 dark:text-gray-400">
            {bookingDateLabel} ·{" "}
            {bookingSelectedTime === "any" ? "כל שעה" : bookingSelectedTime}
          </div>
        </div>
...
<div className="px-8 py-5 border-t border-gray-200 dark:border-commonBorder">
  <button
    type="button"
    className="w-full h-[48px] rounded-full text-sm font-semibold flex items-center justify-center bg-black text-white dark:bg.white dark:text-black hover:opacity-90 transition disabled:opacity-50 disabled:cursor-not-allowed"
    disabled={!bookingSelectedService || !selectedWaitlistClient}
    onClick={() => {
      const isoDate =
      ...
      if (addFlowMode === "waitlist") {
        // 👉 הפלואו נפתח מפופ־אפ רשימת המתנה → להוסיף ל־Waitlist
        // Add to waitlist with full client and service objects, date, and time
        if (selectedWaitlistClient && selectedServiceObj) {
          const newWaitlistItem = {
            ...
            time: bookingSelectedTime || "any",
            status: "waiting",
            createdAt: new Date(),
          };

          setWaitlistItems((prev) => [newWaitlistItem, ...prev]);

          setWaitlistFilter("waiting");
          setWaitlistRange("30days");
          setIsWaitlistOpen(true);

          // ניקוי וסגירת פאנל – בסוף תמיד
          setIsWaitlistAddOpen(false);
          setWaitlistAddStep("date");
          setBookingSelectedTime("any");
          setBookingSelectedService(null);
        ...
        // 👉 הפלואו נפתח מכפתור "הוסף" ביומן → ליצור תור ביומן
        const clientName =
        ...
        const defaultStaff = STAFF_DAY_CALENDARS[0]?.id || "Dana";
        ...
        // ניקוי וסגירת פאנל – בסוף תמיד (רק עבור calendar flow)
        setIsWaitlistAddOpen(false);
        setWaitlistAddStep("date");
        setBookingSelectedTime("any");
        setBookingSelectedService(null);
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" dir="ltr">
      ...
      >
        ...
        {/* Body */}
        <div className="flex-1 px-8 pb-6 pt-1 text-sm text-gray-800 dark:text-gray-100 space-y-6">
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-gray-500">
              <span>גודל התאים ביומן</span>
            ...
            {/* פס התקדמות ויזואלי */}
            <div className="w-full h-2 rounded-full bg-gray-200 dark:bg-[#222] overflow-hidden">
              <div
        ...
        {/* Footer */}
        <div className="px-8 py-5 border-t border-gray-200 dark:border-commonBorder flex gap-3">
          <button
            type="button"
            className="flex-1 h-[44px] rounded-full text-sm font-medium border border-gray-300 dark:border-[#333] text-gray-700 dark:text-gray-100 bg-white dark:bg-[#111] hover:bg-gray-50 dark:hover:bg-[#222]"
            onClick={() => {
              setSlotHeight(appliedSlotHeight);
              setIsSettingsOpen(false);
            }}
          ...
          <button
            type="button"
            className="flex-1 h-[44px] rounded-full text-sm font-semibold bg-black text-white dark:bg.white dark:text-black hover:opacity-90 transition"
            onClick={() => {
...
<div className="relative">
  ...
  setAddFlowMode("calendar"); // פלואו שהתחיל מהיומן
  setIsWaitlistAddOpen(true);
}}
      ...
      {/* 🔥 Drawer חדש לקביעת תור בלחיצה על היומן */}
      <AppointmentDrawer
