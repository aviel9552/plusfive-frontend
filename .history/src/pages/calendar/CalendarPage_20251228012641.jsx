...
<div className="relative">
  ...
  {isWaitlistRangeOpen && (
    <div
      dir="rtl"
      className="absolute right-0 mt-2 w-56 rounded-2xl border border-gray-200 dark:border-[#282828] bg-white dark:bg-[#181818] z-30 py-1 text-xs sm:text-sm px-2"
      style={{
        boxShadow:
    ...
    >
      {rangeOptions.map((opt) => {
        ...
        return (
          <button
            key={opt.key}
            className="w-full flex flex-row-reverse items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-[#262626] transition"
            onClick={() => {
              setWaitlistRange(opt.key);
              setIsWaitlistRangeOpen(false);
            }}
          >
            {/* ה־V עכשיו בצד ימין של השורה */}
            {isActive && (
