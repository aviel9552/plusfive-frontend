...
export const useStripeSubscription = (slug) => {
  ...
  const fetchPrices = useCallback(async () => {
    try {
      ...
      // בדבלופמנט לא מציגים טוסט כל פעם
      if (!isDev) {
  ...
  const fetchSubscription = useCallback(async () => {
    ...
    try {
      ...
      // כאן משאירים טוסטים כרגיל – זה לא נורה כל הזמן
  ...
  const handleSubscribe = useCallback(async (priceId, planName, meterId) => {
    ...
    try {
      ...
      if (msg.includes('Authentication required')) {
        ...
        toast.error(msg || 'Failed to start subscription process. Please try again.');
      }
    } finally {
      setLoading(false);
    }
  }, [isAuthenticated, user, validateAuthState, slug]);
  ...
  const handleCancelSubscription = useCallback(async (cancellationReason = '') => {
    ...
    try {
      ...
      if (msg.includes('not found')) {
        ...
        errorMessage = msg || 'Failed to cancel subscription. Please try again.';
      }
