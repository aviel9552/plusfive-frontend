...
function AdminAnalyticsRevenueAndCustomerStatus({ onLoadingChange }) {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        if (revenueImpactsResponse && revenueImpactsResponse.success && revenueImpactsResponse.data) {
          setRevenueImpactsData(revenueImpactsResponse.data);
        } else {
          console.error('Invalid revenue impacts response structure:', revenueImpactsResponse);
          setRevenueImpactsData({});
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (onLoadingChange) {
      const isLoading = revenueImpact.loading || customerStatus.loading || revenueImpactsLoading;
      onLoadingChange(isLoading);
    }
  }, [revenueImpact.loading, customerStatus.loading, revenueImpactsLoading, onLoadingChange]);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-[24px] font-ttcommons">
      <div className="lg:col-span-7">
        {revenueImpact.error ? (
          <div className="flex justify-center items-center h-[360px] bg-white dark:bg-customGray rounded-2xl shadow-lg border border-gray-200/50 dark:border-gray-700/50">
