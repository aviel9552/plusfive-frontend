    ...
    <>
      ...
      <aside className={sidebarClasses}>
        ...
        <div className={`flex items-center h-[85px] px-4 ${isCollapsed ? "justify-center" : "justify-between"}`}>
          ...
          {isCollapsed && (
            <span className="text-2xl text-gray-800 dark:text-white cursor-pointer" onClick={() => navigate("/")}>P</span>
          )}
        </div>

        {/* NAVIGATION */}
        <nav className="flex-1 overflow-y-auto" onClick={() => isMobile && setIsMobileMenuOpen(false)}>
          <ul className={`${isCollapsed ? "px-0" : "px-2"} space-y-3`}>
            {navLinks.map((link) => (
              <SidebarNavItem key={link.to} {...link} isCollapsed={isCollapsed} isRTL={isRTL} />
            ))}
          </ul>
        </nav>
