...
const SidebarNavItem = ({ to, icon: Icon, label, isCollapsed, specialPaths = [], isRTL = false }) => {
    ...
    const getLinkClasses = () => {
        ...
        ${isActive()
                ? `bg-gray-200 dark:bg-[#2C2C2C] ${isRTL ? 'border-r-4 border-r-pink-500' : 'border-l-4 border-l-pink-500'} text-gray-900 dark:text-white`
                : 'text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-[#2C2C2C] hover:text-gray-900 dark:hover:text-white'
            }`;
    };

    const getTooltipClasses = () => {
        return `fixed ${isRTL ? 'right-[4.5rem]' : 'left-[4.5rem]'} px-3 py-2 bg-gray-800 dark:bg-[#2C2C2C] text-white text-sm rounded-md
    ...
    const getIconClasses = () => {
        return `text-22 lg:text-20 ${!isCollapsed && (isRTL ? 'ml-3' : 'mr-3')} ${isActive() ? 'text-gray-900 dark:text-white' : ''}`;
    }
