...
const CustomYAxisTick = ({ x, y, payload }) => (
  <text x={x} y={y} dy={4} fill="#888" fontSize={12} textAnchor="end">
...
function AnalyticsSecontChart({ isReady = true }) {
  if (!isReady) return null; // ← תוספת: לא מרנדר עד שהדף מוכן
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-ttcommons mt-7">
        ...
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={qrData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }} barGap={10} onMouseLeave={() => setHoveredBar(null)}>
              <CartesianGrid strokeDasharray="6 6" vertical={false} stroke={isDarkMode ? "#D1D5DB" : "#000"} strokeOpacity={0.15} />
      ...
      <div className="bg-white dark:bg-customBrown rounded-2xl p-6 border border-gray-200 dark:border-customBorderColor dark:hover:bg-customBlack hover:bg-customBody shadow-md hover:shadow-sm">
        ...
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={ratingData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }} style={{ filter: 'none' }}>
              <CartesianGrid strokeDasharray="6 6" vertical={true} stroke={isDarkMode ? "#D1D5DB" : "#000"} strokeOpacity={0.15} />
